...
class AvailabilityCalendarTest {
    ...
    fun recordAbsence_shouldFail_whenResponsibleNotInRoster() {
        ...
        assertThatThrownBy { calendar.recordAbsence(charles, p1, p1) }
            .isInstanceOf(IllegalArgumentException::class.java)
    }

    @Test
    fun recordAbsence_shouldFail_whenFromAfterTo() {
        val calendar = AvailabilityCalendar.create(setOf(fabio, theo))

        assertThatThrownBy { calendar.recordAbsence(theo, p2, p1) }
            .isInstanceOf(IllegalArgumentException::class.java)
    }
    ...
    @Test
    fun recordAbsence_shouldFail_whenSameAbsenceAlreadyRecorded() {
        val calendar = AvailabilityCalendar.create(setOf(fabio, theo))
            .recordAbsence(theo, p1, p2)

        assertThatThrownBy { calendar.recordAbsence(theo, p1, p2) }
            .isInstanceOf(IllegalArgumentException::class.java)
    }
    ...
    fun cancelAbsence_shouldFail_whenResponsibleNotInRoster() {
        ...
        assertThatThrownBy { calendar.cancelAbsence(charles, p1, p1) }
            .isInstanceOf(IllegalArgumentException::class.java)
    }
    ...
    fun cancelAbsence_shouldFail_whenAbsenceDoesNotExist() {
        ...
        assertThatThrownBy { calendar.cancelAbsence(theo, p1, p2) }
            .isInstanceOf(IllegalArgumentException::class.java)
    }
    ...
    fun consumeEvents_shouldContainEventsRecorded_andCancelled() {
        val calendar = AvailabilityCalendar.create(setOf(fabio, theo))
            .recordAbsence(theo, p1, p2)
            .cancelAbsence(theo, p1, p2)

        val (clean, events) = calendar.consumeEvents()

        assertThat(events).hasSize(2)
        assertThat(events[0]).isEqualTo(AbsenceRecorded(responsible = theo, from = p1, to = p2))
        assertThat(events[1]).isEqualTo(AbsenceCancelled(responsible = theo, from = p1, to = p2))
